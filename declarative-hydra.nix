...
let
  hydra-jobset-common = {
    ...
    checkinterval = lib.mkOption {
      description =
        "The jobset will be evaluated at `t=0, checkinterval, checkinterval*2, ...`. `0` disables polling.";
      type = types.int;
      default = 0;
    };
  ...
  hydra-declarative-input = types.submodule {
    options = {
      type = lib.mkOption {
        description =
          "The type of the declarative input (i.e. `git`, `string`, `githubpulls`...)";
        type = types.str;
      };
      value = lib.mkOption {
        description =
          "The value of the declarative input (i.e. a git URI, a string...)";
        type = types.str;
      };
  ...
  types = lib.types // {
    hydra-project = types.submodule ({ name, ... }: {
      options = {
        ...
        declarative = lib.mkOption {
          ...
          type = types.nullOr (types.submodule {
            options = {
              file = lib.mkOption {
                type = types.str;
                description = "The JSON file describing the project";
              };
    ...
    hydra-jobset = types.either (types.submodule {
      options = hydra-jobset-common // {
        nixexprinput = lib.mkOption {
          description =
            "Which input will be used for evaluating the Nix expression of the jobset";
          type = types.str;
        };
        nixexprpath = lib.mkOption {
          description =
            "Where is the Nix expression for creating the jobset (i.e. `nixexprinput/nixexprpath`)";
          type = types.str;
        };
        inputs = lib.mkOption {
...
in {
  options.services.declarative-hydra = {
    ...
    usersFile = lib.mkOption {
      type = types.path;
      description =
        "Path to a file where each file is of the form [user:hash:role]";
    };
  ...
  config = {
    systemd.services.declarative-hydra = lib.mkIf cfg.enable {
      ...
      path = lib.mkForce
        (config.systemd.services.hydra-evaluator.path ++ [ pkgs.curl pkgs.jq ]);
      environment = lib.filterAttrs (k: _: k != "PATH")
